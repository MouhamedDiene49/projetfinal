...
const Navbar = () => {
  ...
  const { getBasketTotal, itemsCount, totalAmount, basket } = useContext(BasketContext);
        ...
        <div className='container w-100 flex align-center justify-end'>
          {authData?.isLoggedIn ? (
            ...
            <Link to="login" className='mx-4 login-btn flex align-center justify-end text-dark'>
              <FaUser size={14} />
              <span className='mx-2 fs-13 text-uppercase ls-1'>connexion</span>
            </Link>
          )}

          <button type="button" onClick={logout} className='flex align-center justify-end text-dark'>
            <FiLogOut size={14} />
        ...
        <div className='navbar-main-bottom flex align-center justify-between'>
          <div className="navbar-cats-wrapper">
            <div className='navbar-cats-btn flex align-center text-white px-2 py-2' onClick={toggleCategory}>
              <AiOutlineBars />
              <span className="text-uppercase mx-3 fs-13">toutes catégories</span>
              <BsCaretDownFill />
            </div>
